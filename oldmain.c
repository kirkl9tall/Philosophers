...
void 	print_event(t_philo *phi , char *str)
{
	...
	printf("%ld\t%d\t%s\n" , (get_time() -phi->start) , phi->id_philo , str);
	pthread_mutex_unlock(&phi->tba->print);

}
int  check_philo(t_philo *phi)
{	
	pthread_mutex_lock(&phi->tba->die_mutex);
	if (phi->tba->die == 1){
		pthread_mutex_unlock(&phi->tba->die_mutex);
		return (1);
	}
	pthread_mutex_unlock(&phi->tba->die_mutex);
...
void eating(t_philo *phi)
{
	...
	pthread_mutex_lock(&phi->tba->meals_lock);
	phi->last_meal = get_time(); 							//?
	pthread_mutex_unlock(&phi->tba->meals_lock);
	print_event(phi , "is eating");
	// if (phi->time_to_eating > phi->time_too_die){
...
void philo_sleep(t_philo *phi, time_t time)
{
...
void* routine(void* arg)
{
	...
	while (1)
	{
		...
		if (check_philo(phi)== 1)
			break;
		// capture = get_time();
		philo_sleep(phi, phi->tba->time_sleep);
		// phi->time_func = get_time();
		// phi->time_func -= capture;
		if (check_philo(phi) == 1)
			break;
...
void *monitor (void *arg)
{
	...
	while (1)
	{
		...
		pthread_mutex_unlock(&tb->belly);
		pthread_mutex_lock(&tb->meals_lock);
		pthread_mutex_lock(&tb->die_mutex);
		if (((get_time() - philo[i]->last_meal)  > tb->time_die ) && philo[i]->last_meal != 0)
		{
			print_event(philo[i] , "is died");
			tb->die = 1;
			pthread_mutex_unlock(&tb->die_mutex);
			pthread_mutex_unlock(&tb->meals_lock);
			break;
		}
		pthread_mutex_unlock(&tb->die_mutex);
		pthread_mutex_unlock(&tb->meals_lock);
...
void philo_born(t_philinf *tb,t_philo **phi,pthread_mutex_t *forks)
{
	...
	pthread_mutex_init(&tb->die_mutex,NULL);
	pthread_mutex_init(&tb->meals_lock,NULL);
	// if  !=-1 create monitor 
